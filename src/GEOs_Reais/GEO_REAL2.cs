...
namespace GEOs_REAIS
{
    public class GEO_real2 : GEO_real1
    {
        ...
        public int variavel_atual {get; set;}
        ...
        public GEO_real2(double tau, int n_variaveis_projeto, int definicao_funcao_objetivo, List<RestricoesLaterais> restricoes_laterais_variaveis, int step_obter_NFOBs, double std, int tipo_perturbacao_original_ou_SDdireto, int P) : base(tau, n_variaveis_projeto, definicao_funcao_objetivo, restricoes_laterais_variaveis, step_obter_NFOBs, std, tipo_perturbacao_original_ou_SDdireto){
            this.P = P;
            this.variavel_atual = 0;
        }

        
        public override void verifica_perturbacoes(){
                       
            int i = this.variavel_atual;
            ...
            // Console.WriteLine("Atribuindo perturbações cnt={0} em perturbiteracoes cnt={1}", perturbacoes.Count, perturbacoes_da_iteracao.Count);
        }
        ...
        public override void ordena_e_perturba(){
            ...
            // Console.WriteLine("Chegou pra ordenar! count das perturbacoes = {0}", perturbacoes_da_iteracao.Count);

            // Obtem somente as perturbações da variável
            List<Perturbacao> perturbacoes_da_variavel = new List<Perturbacao>();
            perturbacoes_da_variavel = perturbacoes_da_iteracao;//_da_iteracao.Where(p => p.indice_variavel_projeto == i).ToList();
            
            // Console.WriteLine("Chegou pra ordenar! count das perturbacoes = {0}", perturbacoes_da_variavel.Count);
